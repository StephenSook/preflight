...
function fakeApi(opts: { sabotage?: "drop-answer" | "unsigned" | "put-fails" | "get-fails" } = {}) {
...
describe("the Application API install and rollback", () => {
  it("reads the three voice webhooks and the signed-callbacks flag off an application", () => {
    const v = viewOf({ id: "a", name: "n", capabilities: { voice: { signed_callbacks: false, webhooks: { answer_url: { address: "https://x/a", http_method: "GET" }, event_url: { address: "https://x/e" }, fallback_answer_url: { address: "https://x/f", http_method: "POST" } } } } });
  ...
  it("reports a platform refusal, a dropped hook on read-back, and signed callbacks that did not stick, instead of claiming success", async () => {
    ...
    for (const [sabotage, status, fragment] of [["put-fails", 400, "answer_url must be https"], ["drop-answer", 502, "does not carry the webhooks"], ["unsigned", 502, "signed callbacks"]] as const) {
