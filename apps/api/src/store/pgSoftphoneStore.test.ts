...
describe.skipIf(!url)("PgSoftphoneStore (integration)", () => {
  ...
  it("records under the allowance, refuses at it, and counts only the role and the day asked about", async () => {
    expect(await store.tryRecord("judge", "judge-aaaa", at(1), { since, max: 2 })).toBe(true);
    expect(await store.tryRecord("judge", "judge-bbbb", at(2), { since, max: 2 })).toBe(true);
    expect(await store.tryRecord("judge", "judge-cccc", at(3), { since, max: 2 })).toBe(false);
    // The scheduler has no allowance, and a judge row from before the day does not count.
    expect(await store.tryRecord("scheduler", "scheduler", at(4), undefined)).toBe(true);
    expect(await store.tryRecord("judge", "judge-dddd", at(5), { since: at(4), max: 1 })).toBe(true);
