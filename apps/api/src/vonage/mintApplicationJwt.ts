 ...
 */
export function mintApplicationJwt(applicationId: string, privateKeyPem: string, nowMs: number, ttlSeconds = 600): string {
  const b64 = (o: unknown) => Buffer.from(JSON.stringify(o)).toString("base64url");
  const iat = Math.floor(nowMs / 1000);
  const head = b64({ alg: "RS256", typ: "JWT" });
  const body = b64({ application_id: applicationId, iat, exp: iat + ttlSeconds, jti: randomUUID() });
