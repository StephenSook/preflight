/**
 * SHA-256 in plain TypeScript (FIPS 180-4), so the engine has no Node-only import and the same
 * node ids and payload hashes come out in a browser. Verified against node:crypto in hash.test.ts;
 * the ids already stored in production depend on this staying byte-identical.
 */
...
/** Lowercase hex SHA-256 of a string's UTF-8 bytes; identical to node:crypto's for every input. */
